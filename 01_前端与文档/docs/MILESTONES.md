...
- M0–M4 已完成并通过测试：后端 88 个单测 + 9 个端到端用例，页面级离线回归 48 项。
- M5 待办：域名备案、HTTPS、小程序后台配置 `request`/`uploadFile` 合法域名、申请 `wx.getLocation` 接口权限。
- 真机联调注意事项：手机与电脑同一局域网（或连电脑移动热点），真机调试面板里勾选「不校验合法域名」。
