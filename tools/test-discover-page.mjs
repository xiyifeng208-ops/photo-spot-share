...
// 直接读配置里的候选地址，避免把 IP 写死在测试里（换网络后测试会假失败）
const appConfig = require_(resolve(MINIPROGRAM, 'config/index.js'));
const hostOf = (url) => (url || '').replace(/^https?:\/\//, '').split('/')[0];
const PRIMARY_HOST = hostOf(appConfig.apiBaseUrl);
const FALLBACK_HOST = hostOf((appConfig.fallbackApiBaseUrls || [])[0]);
...
globalThis.wx = makeWx({ locationDelayMs: 0, failHost: PRIMARY_HOST });
const pageE = loadDiscoverPage();
pageE.onLoad();
await wait(800);

const feedsE = feedUrls(globalThis.wx);
check('首选地址确实失败过', feedsE.some((u) => u.includes(PRIMARY_HOST)), PRIMARY_HOST);
check(
  '自动换到备选地址并拿到数据',
  feedsE.some((u) => u.includes(FALLBACK_HOST)) && pageE.data.spots.length === 1,
