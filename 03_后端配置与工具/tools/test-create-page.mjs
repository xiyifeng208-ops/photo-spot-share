...
function makeWx({ reverseConfigured, reverseErrorMessage }) {
  ...
  return {
    ...
    getStorageSync: key => key === 'pss_token' ? 'test-token' : key === 'pss_user' ? { id: 'user-test' } : '',
    setStorageSync: () => {},
    removeStorageSync: () => {},
    showToast: () => {},
    showLoading: () => {},
...
pageA.setData({
  ...
  geoMeta: { province: '上海市', city: '上海市', district: null },
  title: '外滩测试机位',
  agreed: true,
...
check('没有高德时使用用户明确选择的城市', createA && createA.data.geo.city === '上海市', `city=${createA && createA.data.geo.city}`);
