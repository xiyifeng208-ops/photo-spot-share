...
//   node tools/verify-local.mjs --db postgres://spot@localhost:55432/spot_test_verify
//   node tools/verify-local.mjs --port 3400
//
// 分层设计：
//   第一层（无需数据库）：依赖安装、类型检查、单元测试、构建、API 启动与 HTTP 行为
//   第二层（需要 PostGIS）：仅独立 spot_test_* 库，迁移及测试夹具，不运行开发种子。
//
...
function stepStaticChecks() {
  ...
  const tests = run(`${resolvePnpm()} test`, { env: { TEST_DATABASE_URL: '', AMAP_KEY: '', CONTENT_CHECK_ENABLED: 'false' } });
...
async function probeDatabase() {
  if (!databaseUrl) return { reachable: false, host: '未指定独立测试库', port: 0 };
...
async function stepDatabasePrepare(probe) {
  ...
  if (migrate.code !== 0) throw new Error('独立测试库迁移失败，停止后续检查');
}
...
async function stepE2e(probe) {
  ...
  const e2e = run(`${resolvePnpm()} test`, {
    env: { TEST_DATABASE_URL: databaseUrl, DATABASE_URL: databaseUrl, AMAP_KEY: '', CONTENT_CHECK_ENABLED: 'false' },
  });
...
function finish() {
  ...
  if (skipped > 0) {
    console.log(
      c.yellow('\n   数据库层未验证：迁移、端到端用例还没跑过。') +
        '\n   补齐方式（任选其一，装好后重跑本脚本会自动接上）：' +
        '\n     A. 装 Docker Desktop，然后执行：' +
        '\n        docker compose -f deploy/docker-compose.yml up -d postgres' +
        '\n     B. 装 PostgreSQL 16 + PostGIS 扩展，再用 --db 指定连接串：' +
        '\n        node tools/verify-local.mjs --db "postgres://用户:密码@localhost:5432/spot_test_verify"',
    );
  }
...
async function main() {
  if (databaseUrl && !/^\/spot_test_[a-z0-9_]+$/.test(new URL(databaseUrl).pathname)) {
    throw new Error('仅允许 --db 指向 spot_test_* 独立测试库，禁止使用日常数据库');
  }
  console.log(c.bold('\n拍照机位分享 · 本地自检'));
